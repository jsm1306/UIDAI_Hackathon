...
df['rolling_mean_7'] = df.groupby(['state','district'])['enrollment'].transform(lambda x: x.shift(7).rolling(7).mean())

# Use shifted rolling means for 14-day window
df['rolling_mean_14'] = df.groupby(['state','district'])['enrollment'].transform(lambda x: x.shift(14).rolling(14).mean())
# Shift before computing rolling std to prevent leakage
df['rolling_std_7'] = df.groupby(['state','district'])['enrollment'].transform(lambda x: x.shift(7).rolling(7).std())

# Shift before computing rolling std for 14-day window
df['rolling_std_14'] = df.groupby(['state','district'])['enrollment'].transform(lambda x: x.shift(14).rolling(14).std())
